...
class DataValidation:
    ...
    def has_numerical_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            number_of_numerical_columns = len(self._schema_config)
            numerical_cols = dataframe.select_dtypes(include=["number"]).columns
            logging.info(
                f"Required number of numerical columns:{number_of_numerical_columns}"
            )
            logging.info(f"Number of numerical columns in Data frame:{len(numerical_cols)}")
            if len(numerical_cols) == number_of_numerical_columns:
                return True
            return False
        except Exception as e:
            raise NetworkSecurityException(e, sys)
